...
class CacheManager:
    ...
    def generate_cache_key(payload: Dict) -> str:
        ...
        try:
            ...
            if "prompt" in payload and payload["prompt"]:
                normalized_payload["prompt_hash"] = hashlib.md5(
            ...
            if (
                normalized_payload["action"] == "select_use_cases"
                and "selected_use_case_ids" in payload
            ):
                normalized_payload["selected_use_case_ids"] = sorted(
                    payload.get("selected_use_case_ids", [])
                )
        ...
        try:
            ...
            try:
                cached_date = datetime.fromisoformat(
                    item.get("cached_at", "1970-01-01T00:00:00")
                )
    ...
    def save_to_cache(cache_key: str, payload: Dict, result: Dict) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Saved to cache: {cache_key}, expires at {end_of_day.isoformat()}"
            )
            return True
