...
class AgenticWAFROrchestrator:
    ...
    def __init__(self):
        ...
        self.consolidated_report_generator = ConsolidatedReportGenerator(
            self.model_manager
        )
        self.session_store = {}
        ...
        try:
            ...
            status_tracker.update_status(
                StatusCheckpoints.INITIATED,
                {
                    ...
                    "custom_prompt_provided": bool(
                        custom_prompt and custom_prompt.strip()
                    ),
    ...
    def _handle_start(
        ...
        print(f"🔬 Starting comprehensive business research for {company_name}...")
        ...
        # Ensure research_data is always a dictionary to avoid AttributeError on .get
        if research_data is None or not isinstance(research_data, dict):
            research_data = {}
            logger.warning(
        ...
        status_tracker.update_status(
            StatusCheckpoints.AGENT_ANALYZING,
            {
                ...
                "enhanced_with_web_scraping": bool(
        ...
        print(f"👤 Extracting company profile for {company_name}...")
        ...
        print(f"🎯 Generating transformation use cases for {company_name}...")
        structured_use_cases = (
            self.dynamic_use_case_generator.generate_dynamic_use_cases(
                company_profile,
                research_data,
                status_tracker,
                parsed_files_content,
        ...
        print(f"✅ Generated {len(structured_use_cases)} structured use cases")

        # Generate consolidated comprehensive report with web scraping citations
        print(f"📄 Generating consolidated report for {company_name}...")
        ...
        for structured_uc in structured_use_cases:
            context_id = getattr(
                structured_uc, "dynamic_id", f"transformation-{len(legacy_use_cases)}"
            )
        ...
        if web_research_data.get("successful_scrapes", 0) > 0:
            enhancement_notes.append(
                f"web intelligence from {web_research_data['successful_scrapes']} sources"
            )
        if parsed_files_content:
        ...
        enhancement_note = (
            f" (Enhanced with {' and '.join(enhancement_notes)})"
        ...
        return {
            ...
            "research_metadata": {
                "method": research_data.get(
                    "research_method", "web_scraping_with_beautiful_soup"
                ),
                ...
                "successful_web_scrapes": research_data.get(
                    "successful_web_scrapes", 0
                ),
                "files_processed": len(files) if files else 0,
                "file_content_used": bool(parsed_files_content),
                "custom_context_used": bool(custom_context),
                "custom_context_type": custom_context.get("context_type")
                if custom_context
                else None,
                "custom_focus_areas": custom_context.get("focus_areas")
                if custom_context
                else [],
            ...
            "custom_context_summary": {
                ...
                "specific_requirements": custom_context.get("specific_requirements")
                if custom_context
                else "",
                "integration_notes": custom_context.get("integration_notes")
                if custom_context
    ...
    def _handle_select_use_cases(
        ...
        if not selected_use_case_ids:
            return {
                "status": "error",
                "message": "No transformation use cases selected",
            }
        ...
        if not session_data:
            return {
                "status": "error",
                "message": 'Session not found. Please start with action: "start"',
            }
        ...
        valid_selected_ids = [
            id for id in selected_use_case_ids if id in context_aligned_ids
        ]
        ...
        selected_use_cases = [
            uc for uc in legacy_use_cases if uc.id in valid_selected_ids
        ]

        logger.info(
            f"Processing {len(selected_use_cases)} selected use cases for {company_name}"
        )
    ...
    def _handle_fetch(
        self, company_name: str, company_url: str, fetch_type: str, **kwargs
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if fetch_type == "status":
                ...
                return {
                    ...
                    "session_id": session_id,
                    "current_status": current_status,
                    "polling_recommended": current_status.get("current_status")
                    not in [
                        StatusCheckpoints.COMPLETED,
                        StatusCheckpoints.ERROR,
                        StatusCheckpoints.USE_CASES_GENERATED,
                ...
                return self._fetch_wafr_report_enhanced(
                    company_name, company_url, selected_use_case_ids
                )
            elif fetch_type == "all":
    ...
    def _fetch_use_cases_enhanced(
        ...
        for session_id, session_data in self.session_store.items():
            try:
                if (
                    session_data.get("company_name", "").lower() == company_name.lower()
                    and "structured_use_cases" in session_data
                ):
        ...
        if matching_sessions:
            ...
            try:
                latest_session = max(
                    matching_sessions, key=lambda x: x[1].get("timestamp", "1970-01-01")
                )
                ...
                return {
                    ...
                    "company_profile": asdict(
                        self._convert_profile_to_legacy(session_data["company_profile"])
                    )
                    if session_data.get("company_profile")
                    else {},
                    "use_cases": [
                        asdict(uc) for uc in session_data.get("legacy_use_cases", [])
                    ],
                    "available_use_case_ids": session_data.get(
                        "dynamic_use_case_ids", []
                    ),
                    "total_use_cases": len(
                        session_data.get("structured_use_cases", [])
                    ),
                    "report_url": session_data.get("report_url", ""),
                    "files_processed": session_data.get("files_processed", 0),
                    "custom_context_used": bool(custom_context),
                    ...
                    "web_scraping_summary": {
                        "enabled": WEB_SCRAPING_AVAILABLE,
                        "urls_scraped": len(research_data.get("urls_scraped", [])),
                        "successful_scrapes": research_data.get(
                            "successful_web_scrapes", 0
                        ),
                    },
                    ...
                    + (
                        f" (enhanced with {session_data.get('files_processed', 0)} files)"
                        if session_data.get("files_processed", 0) > 0
                        else ""
                    )
                    + (
                        f" (custom context: {custom_context.get('context_type', 'general')})"
                        if custom_context
                        else ""
                    )
                    + (
                        f" (web scraping: {research_data.get('successful_web_scrapes', 0)} sources)"
                        if research_data.get("successful_web_scrapes", 0) > 0
                        else ""
                    )
    ...
    def _fetch_wafr_report_enhanced(
        self,
        company_name: str,
        company_url: str,
        selected_use_case_ids: List[str] = None,
    ) -> Dict[str, Any]:
        ...
        for session_id, session_data in self.session_store.items():
            try:
                if (
                    session_data.get("company_name", "").lower() == company_name.lower()
                    and session_data.get("company_url", "") == company_url
                    and "report_url" in session_data
                ):
        ...
        if matching_report_sessions:
            ...
            for session_id, session_data in matching_report_sessions:
                try:
                    ...
                    logger.warning(
                        f"Error processing report for session {session_id}: {e}"
                    )
                    continue
    ...
    def _fetch_all_data_enhanced(
            ...
            try:
                if session_data.get("company_name", "").lower() == company_name.lower():
                    ...
                    session_summary = {
                        ...
                        "selected_use_cases": session_data.get(
                            "selected_use_case_ids", []
                        ),
                        "report_url": session_data.get("report_url", ""),
                        "files_processed": files_processed,
                        ...
                        "company_profile": asdict(session_data.get("company_profile"))
                        if session_data.get("company_profile")
                        else None,
                        "comprehensive_use_case_analysis": True,
                        "enhanced_formatting": True,
        ...
        if all_sessions:
            return {
                ...
                "detailed_use_cases": use_cases_result
                if use_cases_result.get("status") == "found_cached_use_cases"
                else None,
                "detailed_reports": reports_result
                if reports_result.get("status") == "found_cached_reports"
                else None,
    ...
    def _generate_cache_key_for_company(
        self, company_name: str, company_url: str
    ) -> str:
    ...
    def _parse_uploaded_files(
        self, files: List[str], status_tracker: StatusTracker
    ) -> Optional[str]:
        ...
        for i, file_url in enumerate(files):
            try:
                ...
                if content:
                    ...
                    successful_parses += 1
                    logger.info(
                        f"Successfully parsed file {i + 1}: {len(content)} characters"
                    )
                else:
        ...
        if combined_content:
            ...
            logger.info(
                f"Combined file content: {len(result)} total characters from {successful_parses} files"
            )
            return result
    ...
    def _extract_company_profile(
        ...
        # Defensive: ensure research_data is a dictionary
        if research_data is None or not isinstance(research_data, dict):
            research_data = {}

        web_context = ""
        web_research_data = research_data.get("web_research_data") or {}
        if web_research_data.get("research_content"):
            web_context = f"""
            
WEB INTELLIGENCE ANALYSIS:
Based on web scraping of {web_research_data.get("successful_scrapes", 0)} sources:

{web_research_data.get("research_content", "")[:2000]}
