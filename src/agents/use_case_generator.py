...
class DynamicUseCaseGenerator:
    ...
    def generate_dynamic_use_cases(
        self,
        ...
        web_context_section = ""
        if research_data.get("web_research_data") and research_data[
            "web_research_data"
        ...
        if custom_context and custom_context.get("processed_prompt"):
            generation_prompt = (
                CustomPromptProcessor.integrate_prompt_into_use_case_generation(
                    base_generation_prompt, custom_context
                )
            )
            logger.info(
                f"Use case generation prompt enhanced with custom context: {custom_context.get('context_type', 'unknown')}"
            )
        else:
        ...
        try:
            logger.info(
                f"Generating transformation use cases for {company_profile.name}"
            )
            ...
            if research_data.get("web_research_data"):
                ...
                print(
                    f"    🌐 Web research content: {len(web_data.get('research_content', ''))} chars"
                )
                print(
                    f"    ✅ Successful web scrapes: {web_data.get('successful_scrapes', 0)}"
                )
            print(f"  📁 File content available: {bool(parsed_files_content)}")
            if parsed_files_content:
                print(f"    📄 File content length: {len(parsed_files_content)} chars")
            print(
                f"  🎯 Custom context available: {bool(custom_context and custom_context.get('processed_prompt'))}"
            )
            ...
            print(
                f"🌐 Generator agent also has access to http_request tools for additional research"
            )
            ...
            print(
                f"✅ Use case generation complete - response length: {len(response_text)} characters"
            )
            print(
                f"🔍 Generator agent may have fetched additional web content for use case creation"
            )

            # Parse the XML-formatted use cases
            print(f"🔍 Parsing XML-formatted use cases...")
            use_cases = self._parse_xml_formatted_use_cases(
                response_text, company_profile
            )
            print(f"✅ Parsed {len(use_cases)} use cases successfully")

            if not use_cases:
                logger.warning("No use cases parsed from XML, generating fallback")
                use_cases = self._generate_fallback_use_cases(
                    company_profile, research_data, parsed_files_content, custom_context
                )
            ...
            logger.info(
                f"Successfully generated {len(use_cases)} transformation use cases for {company_profile.name}"
            )
            ...
            if status_tracker:
                status_tracker.update_status(
                    ...
                    {
                        "error_type": "use_case_generation_error",
                        "error_message": str(e),
                    },
                )
            return self._generate_fallback_use_cases(
                company_profile, research_data, parsed_files_content, custom_context
            )

    def _parse_xml_formatted_use_cases(
        self, response_text: str, company_profile: CompanyProfile
    ) -> List[UseCaseStructured]:
        ...
        try:
            ...
            use_case_matches = re.findall(
                use_case_pattern, response_text, re.DOTALL | re.IGNORECASE
            )
            ...
            for i, use_case_block in enumerate(use_case_matches):
                try:
                    use_case = self._parse_single_xml_use_case(
                        use_case_block, company_profile, i
                    )
                    if use_case:
            ...
            if len(use_cases) < 5:
                ...
                supplements = self._generate_supplemental_use_cases(
                    company_profile, len(use_cases)
                )
    ...
    def _parse_single_xml_use_case(
        self, use_case_block: str, company_profile: CompanyProfile, index: int
    ) -> Optional[UseCaseStructured]:
        ...
        def extract_list_content(
            tag_name: str, content: str, default: List[str] = None
        ) -> List[str]:
            if default is None:
                default = []
        ...
        try:
            ...
            use_case_id = extract_tag_content(
                "id", use_case_block, f"business-transformation-{index}"
            )
            ...
            title = (
                ...
                else f"Business Transformation Initiative {index + 1}"
            )

            # Use description as proposed solution if available
            category = extract_tag_content(
                "category", use_case_block, "Business Optimization"
            )
            current_state = extract_tag_content(
                "current_state",
                use_case_block,
                "Current business processes with optimization opportunities",
            )
            ...
            if description and len(description.strip()) > 0:
                ...
                proposed_solution = extract_tag_content(
                    "proposed_solution",
                    use_case_block,
                    "Strategic transformation solution with technology enablers",
                )

            aws_services = extract_list_content(
                "aws_services", use_case_block, ["Lambda", "S3", "CloudWatch"]
            )
            business_value = extract_tag_content(
                "business_value",
                use_case_block,
                "Enhanced business performance and competitive advantage",
            )
            implementation_phases = extract_list_content(
                "implementation_phases",
                use_case_block,
                ["Assessment", "Design", "Implementation", "Optimization"],
            )
            timeline_months = extract_int_content("timeline_months", use_case_block, 6)
            monthly_cost_usd = extract_int_content(
                "monthly_cost_usd", use_case_block, 3000
            )
            ...
            success_metrics = extract_list_content(
                "success_metrics",
                use_case_block,
            ...
            if not proposed_solution or len(proposed_solution) < 10:
                logger.warning(
                    f"Use case {index} has invalid description/proposed_solution"
                )
            ...
            logger.info(
                f"Parsed use case with name: '{title}' and description length: {len(proposed_solution)}"
            )
    ...
    def _generate_supplemental_use_cases(
        self, company_profile: CompanyProfile, current_count: int
    ) -> List[UseCaseStructured]:
        ...
        for i in range(min(needed_count, len(business_supplements))):
            ...
            # Add dynamic ID
            supplement.dynamic_id = (
                f"business-transformation-supplement-{current_count + i + 1}"
            )
            supplements.append(supplement)

        return supplements

    def _generate_fallback_use_cases(
        self,
        company_profile: CompanyProfile,
        research_data: Dict[str, Any],
        parsed_files_content: str = None,
