...
class CompanyResearchSwarm:
    ...
    def conduct_comprehensive_research(
            ...
            if status_tracker:
                status_tracker.update_status(
                    StatusCheckpoints.WEB_SCRAPING_COMPLETED,
                    {
                        ...
                        "successful_scrapes": web_research_data.get(
                            "successful_scrapes", 0
                        ),
                        "total_attempts": web_research_data.get(
        ...
        if custom_context and custom_context.get("processed_prompt"):
            research_prompt = CustomPromptProcessor.integrate_prompt_into_research(
                base_research_prompt, custom_context
            )
            logger.info(
                f"Research prompt enhanced with custom context: {custom_context.get('context_type', 'unknown')}"
            )
        else:
        ...
        try:
            ...
            if web_research_data:
                print(
                    f"    ✅ Successful web scrapes: {web_research_data.get('successful_scrapes', 0)}"
                )
                print(
                    f"    📄 Web content length: {len(web_research_data.get('research_content', ''))}"
                )
            ...
            if status_tracker:
                status_tracker.update_status(
                    StatusCheckpoints.RESEARCH_IN_PROGRESS,
                    {
                        ...
                        "using_custom_context": bool(
                            custom_context and custom_context.get("processed_prompt")
                        ),
                    },
                    current_agent="research_coordinator",
                    urls_scraped=web_research_data.get("urls_scraped", [company_url])
                    if web_research_data
                    else [company_url],
                )
            ...
            print(
                f"✅ Research analysis complete - result length: {len(str(research_result))} characters"
            )
            print(f"🔍 Agent has completed web content fetching")

            # Get URLs from web research
            scraped_urls = (
                web_research_data.get("urls_scraped", []) if web_research_data else []
            )
            ...
            if web_research_data:
                enhancement_notes.append(
                    f"Web Intelligence from {web_research_data.get('successful_scrapes', 0)} sources"
                )
            if parsed_files_content:
            ...
            enhancement_note = (
                f" - Enhanced with: {', '.join(enhancement_notes)}"
            ...
            if status_tracker:
                status_tracker.update_status(
                    StatusCheckpoints.RESEARCH_COMPLETED,
                    {
                        ...
                        "successful_web_scrapes": web_research_data.get(
                            "successful_scrapes", 0
                        )
                        if web_research_data
                        else 0,
                        ...
                        "custom_context_integrated": bool(
                            custom_context and custom_context.get("processed_prompt")
                        ),
                    },
            ...
            if web_research_data:
                print(
                    f"🌐 BeautifulSoup web scraping attempted: {web_research_data.get('total_urls_attempted', 0)} URLs"
                )
                print(
                    f"✅ BeautifulSoup successful scrapes: {web_research_data.get('successful_scrapes', 0)}"
                )
                ...
                print(
                    f"⚠️  BeautifulSoup web scraping: Not available (libraries missing)"
                )
            ...
            return {
                ...
                "successful_web_scrapes": web_research_data.get("successful_scrapes", 0)
                if web_research_data
                else 0,
                "strands_http_requests": True,
                "agent_web_access": True,
                "file_content_used": bool(parsed_files_content),
                "file_content_length": len(parsed_files_content)
                if parsed_files_content
                else 0,
                "custom_context_used": bool(
                    custom_context and custom_context.get("processed_prompt")
                ),
    ...
    def _create_fallback_research(
        ...
        parsed_files_content: str = None,
        custom_context: Dict[str, str] = None,
        web_research_data: Dict[str, Any] = None,
    ) -> Dict[str, Any]:
