"""
PowerPoint generation service integrated into the src codebase.

This module embeds the multi-template PPT generator and provides a helper to
create a PPT from a generated report PDF stored in S3, uploading the result
back to S3 and returning its URL.
"""
...
class FirstDeckTemplate(PPTTemplate):
    ...
    def _create_opportunity_slide(
        self, ppt: Presentation, data: Dict[str, Any]
    ) -> None:
        slide = ppt.slides.add_slide(ppt.slide_layouts[1])
        title = slide.shapes.title
        content = slide.placeholders[1]
...
class MarketingTemplate(PPTTemplate):
    ...
    def get_analysis_prompt(self, content: str, company_name: str) -> str:
        return f"""
            ...
            {{
                ...
                "slides": [
                    {{"type": "title", "title": "Unlock Your Business Potential", "subtitle": "Digital Transformation Success with {company_name}"}},
                    {{"type": "problem_agitation", "title": "The Challenge Every Business Faces", "pain_points": ["Specific business pain point with emotional impact", "Cost of inaction or competitive disadvantage", "Frustrations with current processes"], "urgency": "Why immediate action is needed"}},
                    {{"type": "solution_story", "title": "The Transformation Solution", "story_elements": ["How transformation addresses core challenges", "Unique approach and differentiators", "Technology enablement and innovation"]}},
                    {{"type": "benefits_showcase", "title": "Real Results You Can Achieve", "benefit_categories": ["Operational Excellence: Specific improvements", "Financial Impact: Cost savings and revenue growth", "Competitive Advantage: Market positioning benefits"]}},
                    {{"type": "success_scenarios", "title": "Success Stories and Use Cases", "scenarios": ["Scenario 1: Department/process transformation", "Scenario 2: Technology innovation success", "Scenario 3: Market expansion achievement"]}},
                    {{"type": "call_to_action", "title": "Start Your Transformation Journey", "action_items": ["Schedule transformation assessment", "Pilot program opportunity", "Partnership discussion"]}}
                ]
            }}
    ...
    def _style_marketing_slide(
        self, title, content, accent_color=False, success_color=False
    ) -> None:
        ...
        text_color = (
            self.colors["accent"]
            if accent_color
            else (self.colors["success"] if success_color else self.colors["text"])
        )
        for paragraph in content.text_frame.paragraphs:
    ...
    def get_analysis_prompt(self, content: str, company_name: str) -> str:
        return f"""
            ...
            {{
                ...
                "slides": [
                    {{"type": "title", "title": "Use Case Implementation Strategy", "subtitle": "{company_name} Transformation Scenarios"}},
                    {{"type": "use_case_overview", "title": "Use Case Portfolio Overview", "use_case_summary": ["Use Case 1: Process automation and efficiency", "Use Case 2: Data analytics and insights", "Use Case 3: Customer experience enhancement", "Use Case 4: Operational cost optimization"]}},
                    {{"type": "detailed_use_case", "use_case_number": 1, "title": "Use Case 1: [Specific Process Name]", "current_state": "Detailed description of current challenge", "proposed_solution": "Specific technology and process solution", "implementation_steps": ["Phase 1: Assessment and planning (months 1-2)", "Phase 2: Pilot implementation (months 3-4)", "Phase 3: Full rollout (months 5-8)"], "expected_benefits": ["Quantified efficiency improvement", "Cost reduction estimate", "Quality improvement metric"], "success_metrics": "How success will be measured"}},
                    {{"type": "implementation_approach", "title": "Implementation Methodology", "methodology_phases": ["Discovery and requirements gathering", "Solution design and architecture", "Pilot development and testing", "Training and change management", "Full deployment and optimization"], "timeline": "12-18 month implementation cycle"}},
                    {{"type": "risk_mitigation", "title": "Risk Management and Success Factors", "risk_factors": ["Technical integration challenges", "User adoption and change resistance", "Data migration and quality issues"], "mitigation_strategies": ["Phased implementation approach", "Comprehensive training program", "Dedicated project management office"]}}
                ]
            }}
    ...
    def get_analysis_prompt(self, content: str, company_name: str) -> str:
        return f"""
            ...
            {{
                ...
                "slides": [
                    {{"type": "title", "title": "Technical Architecture Overview", "subtitle": "{company_name} System Design and Implementation"}},
                    {{"type": "architecture_overview", "title": "System Architecture", "architecture_components": ["Frontend layer: User interface and experience", "Backend services: Business logic and APIs", "Data layer: Storage and analytics platform", "Integration layer: External system connectivity"], "design_principles": ["Scalable and modular architecture", "Cloud-native deployment approach", "Microservices-based design pattern"]}},
                    {{"type": "technology_stack", "title": "Technology Stack and Platforms", "frontend_tech": ["React/Vue.js", "TypeScript", "Responsive design"], "backend_tech": ["Node.js/Python", "REST APIs", "Microservices"], "database_tech": ["PostgreSQL/MongoDB", "Redis caching", "Data warehouse"], "cloud_platform": ["AWS/Azure", "Container orchestration", "CI/CD pipeline"]}},
                    {{"type": "integration_specs", "title": "Integration Architecture", "integration_patterns": ["API-first design approach", "Event-driven architecture", "Real-time data synchronization"], "data_flow": ["Data ingestion and validation", "Processing and transformation", "Storage and retrieval optimization"]}},
                    {{"type": "performance_security", "title": "Performance and Security", "performance_requirements": ["Response time: <200ms for API calls", "Throughput: 10,000+ concurrent users", "Availability: 99.9% uptime SLA"], "security_measures": ["End-to-end encryption", "Multi-factor authentication", "Regular security audits and penetration testing"]}}
                ]
            }}
...
class StrategyTemplate(PPTTemplate):
    def __init__(self):
        colors = {
            ...
            "highlight": RGBColor(245, 101, 101),
        }
    ...
    def get_analysis_prompt(self, content: str, company_name: str) -> str:
        return f"""
            ...
            {{
                ...
                "slides": [
                    {{"type": "title", "title": "Strategic Transformation Roadmap", "subtitle": "{company_name} 3-Year Strategic Plan"}},
                    {{"type": "current_state", "title": "Current State Assessment", "strengths": ["Key organizational strengths", "Competitive advantages", "Market position benefits"], "challenges": ["Strategic challenges to address", "Market pressures and threats", "Internal capability gaps"]}},
                    {{"type": "strategic_vision", "title": "Strategic Vision and Objectives", "vision_statement": "Clear vision for future state", "strategic_objectives": ["Objective 1: Market leadership and growth", "Objective 2: Operational excellence", "Objective 3: Innovation and transformation"]}},
                    {{"type": "strategic_initiatives", "title": "Strategic Initiatives Portfolio", "initiative_categories": ["Growth Initiatives: Market expansion and new products", "Efficiency Initiatives: Process optimization and cost reduction", "Innovation Initiatives: Technology and capability building"], "prioritization": "Based on impact, feasibility, and strategic alignment"}},
                    {{"type": "implementation_roadmap", "title": "3-Year Implementation Roadmap", "year_1": ["Foundation building and quick wins", "Core capability development", "Initial market expansion"], "year_2": ["Scale successful initiatives", "Technology platform enhancement", "Operational optimization"], "year_3": ["Market leadership achievement", "Innovation commercialization", "Sustainable competitive advantage"]}},
                    {{"type": "governance_metrics", "title": "Success Metrics and Governance", "success_metrics": ["Financial performance indicators", "Market share and customer metrics", "Operational efficiency measures"], "governance_structure": "Strategic oversight and review process"}}
                ]
            }}
    ...
    def _create_current_state_slide(
        self, ppt: Presentation, data: Dict[str, Any]
    ) -> None:
        slide = ppt.slides.add_slide(ppt.slide_layouts[1])
        title = slide.shapes.title
...
class BedrockAnalyzer:
    ...
    def analyze_with_template(
        self, content: str, template: PPTTemplate, company_name: str
    ) -> Dict[str, Any]:
    ...
    def _create_fallback_structure(
        self, template_name: str, company_name: str
    ) -> Dict[str, Any]:
        return {
            "presentation_info": {
...
class MultiTemplatePPTGenerator:
    ...
    def generate_presentation(
        ...
        if template_type not in TEMPLATE_REGISTRY:
            raise ValueError(
                f"Invalid template type. Choose from: {list(TEMPLATE_REGISTRY.keys())}"
            )
        output_dir = output_dir or LAMBDA_TMP_DIR or "."
        os.makedirs(output_dir, exist_ok=True)
        content = self.pdf_extractor.extract_text(pdf_path)
        template = TEMPLATE_REGISTRY[template_type]()
        structure = self.bedrock_analyzer.analyze_with_template(
            content, template, company_name or ""
        )
        ppt = Presentation()
...
def generate_ppt_from_s3_report(
    report_s3_url: str,
    company_name: str,
    template_type: str = "use_case",
    aws_region: str = "us-east-1",
) -> Optional[str]:
    """Generate a PPT from an S3-hosted report PDF and upload to S3.

    Args:
        report_s3_url: S3 URL or https S3 URL to the generated report PDF
        company_name: Name to use for personalization and naming
        template_type: One of TEMPLATE_REGISTRY keys
        aws_region: AWS region for Bedrock

    Returns:
        Public HTTPS URL of the uploaded PPT in S3, or None on failure
    """
    try:
        if not PPT_AVAILABLE:
